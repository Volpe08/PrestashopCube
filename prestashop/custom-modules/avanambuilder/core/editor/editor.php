...
namespace AvanamBuilder\Core\Editor;

require_once AVANAM_BUILDER_PATH . 'src/Wp_Editors.php';

use AvanamBuilder\Src\_WP_Editors;
use AvanamBuilder\Core\Common\Modules\Ajax\Module as Ajax;
use AvanamBuilder\Core\Debug\Loading_Inspection_Manager;
use AvanamBuilder\Core\Responsive\Responsive;
use AvanamBuilder\Core\Settings\Manager as SettingsManager;
use AvanamBuilder\Icons_Manager;
use AvanamBuilder\Plugin;
use AvanamBuilder\Schemes_Manager;
use AvanamBuilder\Settings;
use AvanamBuilder\Shapes;
use AvanamBuilder\TemplateLibrary\Source_Local;
use AvanamBuilder\Tools;
use AvanamBuilder\User;
use AvanamBuilder\Utils;
...
class Editor {
	...
	public function print_editor_template() {
		include AVANAM_BUILDER_PATH . 'includes/editor-templates/editor-wrapper.php';
	}
	...
	public function enqueue_scripts() {
		...
		$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG || defined( 'AVANAM_BUILDER_TESTS' ) && AVANAM_BUILDER_TESTS ) ? '' : '.min';

		Wp_Helper::wp_register_script(
			'elementor-editor-modules',
			AVANAM_BUILDER_ASSETS_URL . 'js/editor-modules' . $suffix . '.js',
			[
				'elementor-common-modules',
			],
			AVANAM_BUILDER_VERSION,
			true
		);
		// Hack for waypoint with editor mode.
		Wp_Helper::wp_register_script(
			'elementor-waypoints',
			AVANAM_BUILDER_ASSETS_URL . 'lib/waypoints/waypoints-for-editor.js',
			[
				'jquery',
			],
			'4.0.2',
		...
		Wp_Helper::wp_register_script(
			'perfect-scrollbar',
			AVANAM_BUILDER_ASSETS_URL . 'lib/perfect-scrollbar/js/perfect-scrollbar' . $suffix . '.js',
			[],
			'1.4.0',
			true
		);
		...
		Wp_Helper::wp_register_script(
			'jquery-easing',
			AVANAM_BUILDER_ASSETS_URL . 'lib/jquery-easing/jquery-easing' . $suffix . '.js',
			[
				'jquery',
			],
			'1.3.2',
		...
		Wp_Helper::wp_register_script(
			'nprogress',
			AVANAM_BUILDER_ASSETS_URL . 'lib/nprogress/nprogress' . $suffix . '.js',
			[],
			'0.2.0',
			true
		);
		...
		Wp_Helper::wp_register_script(
			'tipsy',
			AVANAM_BUILDER_ASSETS_URL . 'lib/tipsy/tipsy' . $suffix . '.js',
			[
				'jquery',
			],
			'1.0.0',
		...
		Wp_Helper::wp_register_script(
			...
			AVANAM_BUILDER_ASSETS_URL . 'lib/e-select2/js/e-select2.full' . $suffix . '.js',
			[
				'jquery',
			],
			'4.0.6-rc.1',
		...
		Wp_Helper::wp_register_script(
			'flatpickr',
			AVANAM_BUILDER_ASSETS_URL . 'lib/flatpickr/flatpickr' . $suffix . '.js',
			[
				'jquery',
			],
			'1.12.0',
		...
		Wp_Helper::wp_register_script(
			'jquery-hover-intent',
			AVANAM_BUILDER_ASSETS_URL . 'lib/jquery-hover-intent/jquery-hover-intent' . $suffix . '.js',
			[],
			'1.0.0',
			true
		);
		...
		Wp_Helper::wp_register_script(
			'nouislider',
			AVANAM_BUILDER_ASSETS_URL . 'lib/nouislider/nouislider' . $suffix . '.js',
			[],
			'13.0.0',
			true
		);
		...
		Wp_Helper::wp_register_script(
			'autocomplete',
			AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/autocomplete.min.js',
			[
				'jquery',
			],
			'1.11.4',
		...
		Wp_Helper::wp_register_script(
			'imagesloaded',
			AVANAM_BUILDER_ASSETS_URL . 'lib/wp-include/imagesloaded.min.js',
			[],
			'3.2.0',
			true
		);
		...
		Wp_Helper::wp_register_script(
			'ava-editor',
			AVANAM_BUILDER_ASSETS_URL . 'js/ava-editor' . $suffix . '.js',
			[
			],
			'',
			true
		...
		Wp_Helper::wp_register_script(
			'elementor-editor',
			AVANAM_BUILDER_ASSETS_URL . 'js/editor' . Wp_Helper::get_elementor_editor_suffix() . $suffix . '.js',
			[
				'elementor-common',
				'elementor-editor-modules',
				'jquery-ui-sortable',
		...
		$config = [
			'version' => AVANAM_BUILDER_VERSION,
			'home_url' => __PS_BASE_URI__,
			'data' => $editor_data,
			'document' => $data['document'],
			'autosave_interval' => AVANAM_AUTOSAVE_INTERVAL,
			...
			'icons' => [
				...
				'goProURL' => Utils::get_pro_link( 'https://avanam.org/presta/pro/?utm_source=icon-library-go-pro&utm_campaign=gopro&utm_medium=wp-dash' ),
			],
			'fa4_to_fa5_mapping_url' => AVANAM_BUILDER_ASSETS_URL . 'lib/font-awesome/migration/mapping.json',
			...
			'settings_page_link' => Wp_Helper::get_exit_to_dashboard( 'AdminAvanamBuilderSettings' ),
			'tools_page_link' =>  Wp_Helper::get_exit_to_dashboard( 'AdminAvanamBuilderSettings' ),
			'elementor_site' => 'https://avanam.org/presta/about-us/',
			'docs_elementor_site' => 'https://avanam.org/presta/docs/',
			'help_the_content_url' => $data['document']['urls']['preview'] . '&preview-debug',
			'help_right_click_url' => 'https://avanam.org/presta/meet-right-click/',
			'help_flexbox_bc_url' => 'https://avanam.org/presta/flexbox-layout-bc/',
			...
			'preview' => [
				'help_preview_error_url' => 'https://avanam.org/presta/preview-not-loaded/',
				'help_preview_http_error_url' => 'https://avanam.org/presta/preview-not-loaded/#permissions',
				'help_preview_http_error_500_url' => 'https://avanam.org/presta/500-error/',
			...
			'i18n' => [		
				'elementor' => Wp_Helper::__( 'AvanamBuilder', 'elementor' ),
				...
				'flexbox_attention_message' => Wp_Helper::__( 'AvanamBuilder 1.0.0 introduces key changes to the layout using CSS Flexbox. Your existing pages might have been affected, please review your page before publishing.', 'elementor' ),

				// Menu.
				'about_elementor' => Wp_Helper::__( 'About AvanamBuilder', 'elementor' ),
				...
				'dialog_confirm_enable_svg' => Wp_Helper::__( 'Before you enable SVG upload, note that SVG files include a security risk. AvanamBuilder does run a process to remove possible malicious code, but there is still risk involved when using such files.', 'elementor' ),

				// Enable fontawesome 5 if needed.
				'enable_fa5' => Wp_Helper::__( 'AvanamBuilder\'s New Icon Library', 'elementor' ),
				'dialog_confirm_enable_fa5' => Wp_Helper::__( 'AvanamBuilder v1.0 includes an upgrade from Font Awesome 4 to 5. In order to continue using icons, be sure to click "Upgrade".', 'elementor' ) . ' <a href="https://avanam.org/presta/fontawesome-migration/" target="_blank">' . Wp_Helper::__( 'Learn More', 'elementor' ) . '</a>',
				...
				'device_incompatible_message' => Wp_Helper::__( 'Your browser isn\'t compatible with all of AvanamBuilder\'s editing features. We recommend you switch to another browser like Chrome or Firefox.', 'elementor' ),
				...
				'preview_el_not_found_message' => Wp_Helper::__( 'You must call \'the_content\' function in the current template, in order for AvanamBuilder to work on this page.', 'elementor' ),
			...
			'elementPromotionURL' => Wp_Helper::get_exit_to_dashboard( 'AdminAvanamBuilderLicense' ),
			'customIconsURL' => Wp_Helper::get_exit_to_dashboard( 'AdminAvanamBuilderLicense' ),
			'library_connect' => [
				'is_connected' => false
			],
	...
	public function enqueue_styles() {
		...
		Wp_Helper::wp_register_style(
			'font-awesome',
			AVANAM_BUILDER_ASSETS_URL . 'lib/font-awesome/css/font-awesome' . $suffix . '.css',
			[],
			'4.7.0'
		);
		...
		Wp_Helper::wp_register_style(
			'line-awesome',
			AVANAM_BUILDER_ASSETS_URL . 'lib/line-awesome/line-awesome.min.css',
			[],
			'1.3.0'
		);
		...
		Wp_Helper::wp_register_style(
			'pe-icon',
			AVANAM_BUILDER_ASSETS_URL . 'lib/pe-icon/Pe-icon-7-stroke.min.css',
			[],
			'1.2.0'
		);
		...
		Wp_Helper::wp_register_style(
			'elementor-select2',
			AVANAM_BUILDER_ASSETS_URL . 'lib/e-select2/css/e-select2' . $suffix . '.css',
			[],
			'4.0.6-rc.1'
		);
		...
		Wp_Helper::wp_register_style(
			'flatpickr',
			AVANAM_BUILDER_ASSETS_URL . 'lib/flatpickr/flatpickr' . $suffix . '.css',
			[],
			'1.12.0'
		);
		...
		Wp_Helper::wp_register_style(
			'elementor-common',
			AVANAM_BUILDER_ASSETS_URL . 'css/common' . $suffix . '.css',
			[],
			AVANAM_BUILDER_VERSION
		);
		...
		Wp_Helper::wp_register_style(
			'elementor-editor',
			AVANAM_BUILDER_ASSETS_URL . 'css/editor' . $direction_suffix . $suffix . '.css',
			[
				'font-awesome',
				'elementor-common',
				'elementor-select2',
		...
		Wp_Helper::wp_register_style(
			'elementor-ava-editor',
			AVANAM_BUILDER_ASSETS_URL . 'css/ava-editor' . $direction_suffix . $suffix . '.css',
			[],
			AVANAM_BUILDER_VERSION
		);
		
		Wp_Helper::wp_enqueue_style( 'elementor-ava-editor' );
		...
		if ( Responsive::has_custom_breakpoints() ) {
			...
			wp_add_inline_style( 'elementor-editor', '.elementor-device-laptop #elementor-preview-responsive-wrapper { width: ' . $breakpoints['lg'] . 'px; }' );
		}
	...
	private function init_editor_templates() {
		...
		foreach ( $template_names as $template_name ) {
			Plugin::$instance->common->add_template( AVANAM_BUILDER_PATH . "includes/editor-templates/$template_name.php" );
		}
	}
    ...
    private function get_elementor_data() {
		...
		$exit_to_dashboard = Wp_Helper::get_exit_to_dashboard('AdminAvanamBuilderHome');
		
		$content = [];
		
        switch ( $post_type ) {
            case 'header':
				$params = [ 'id_avanam_builder_post' => $id_post, 'updateavanam_builder_post' => 1 ];
				$exit_to_dashboard =  Wp_Helper::get_exit_to_dashboard('AdminAvanamBuilderHeader', $params);
                break;
            case 'footer':
				$params = [ 'id_avanam_builder_post' => $id_post, 'updateavanam_builder_post' => 1 ];
				$exit_to_dashboard =  Wp_Helper::get_exit_to_dashboard('AdminAvanamBuilderFooter', $params);
                break;
            case 'home':
				$params = [ 'id_avanam_builder_post' => $id_post, 'updateavanam_builder_post' => 1 ];
				$exit_to_dashboard =  Wp_Helper::get_exit_to_dashboard('AdminAvanamBuilderHome', $params);
                break;
            case 'hook':
				$related = Wp_Helper::getRelatedByIdPost();
				$params = [ 'id_avanam_builder_related' => $related['id_avanam_builder_related'], 'updateavanam_builder_related' => 1 ];
				$exit_to_dashboard =  Wp_Helper::get_exit_to_dashboard('AdminAvanamBuilderHook', $params);
                break;
            case 'category':
		...
		$obj = new \AvanamBuilderPost($id_post, $id_lang);
