...
namespace AvanamBuilder\Core\Revisions;
...
class Revisions_Manager {
    ...
    public static function get_list_revisions() {	
        $sql = 'SELECT * FROM `'._DB_PREFIX_.'avanam_builder_revisions` 
		...
		foreach ( $list as $key => $post ) {
			$revision = new \AvanamBuilderRevisions( $post['id_avanam_builder_revisions'] );
	...
	public static function get_revisions( $post_id = 0, $query_args = [], $parse_result = true ) {
		...
		foreach ( $posts as $post ) {			
			...
			$revisions[] = [
				...
				'type' => $type,
				'gravatar' => '<img alt="" src="' . AVANAM_BUILDER_ASSETS_URL . 'images/avata.jpg' . '" width="22" height="22">',
			];
		}
	...
	public static function ajax_get_revision_data( array $data ) {
		...
		if( $data['id'] == 'current' ){
			$revision = new \AvanamBuilderPost( Wp_Helper::$id_post, $id_lang );

			$settings = SettingsManager::get_settings_managers( 'page' )->get_model( Wp_Helper::$id_post )->get_data( 'settings' );	
		}else{
			$revision = new \AvanamBuilderRevisions( $data['id'] );
