...
define( 'AVANAM_BUILDER_VERSION', '1.4.9' );
define( 'AVANAM_BUILDER_PREVIOUS_STABLE_VERSION', '1.4.8' );

define( 'AVANAM_BUILDER__FILE__', __FILE__ );
define( 'AVANAM_BUILDER_PLUGIN_BASE', 'avanambuilder/avanambuilder.php' );
define( 'AVANAM_BUILDER_PATH', _PS_MODULE_DIR_ . 'avanambuilder/' );

if ( defined( 'AVANAM_BUILDER_TESTS' ) && AVANAM_BUILDER_TESTS ) {
    define( 'AVANAM_BUILDER_URL', 'file://' . AVANAM_BUILDER_PATH );
} else {
    define( 'AVANAM_BUILDER_URL', \Context::getContext()->shop->getBaseURL(true) . 'modules/avanambuilder/' );
}

define( 'AVANAM_BUILDER_ASSETS_PATH', AVANAM_BUILDER_PATH . 'assets/' );
define( 'AVANAM_BUILDER_ASSETS_URL', AVANAM_BUILDER_URL . 'assets/' );

define( 'AVANAM_MINUTE_IN_SECONDS', 60 );
define( 'AVANAM_HOUR_IN_SECONDS', 60 * AVANAM_MINUTE_IN_SECONDS );
define( 'AVANAM_DAY_IN_SECONDS', 24 * AVANAM_HOUR_IN_SECONDS );
define( 'AVANAM_WEEK_IN_SECONDS', 7 * AVANAM_DAY_IN_SECONDS );
define( 'AVANAM_MONTH_IN_SECONDS', 30 * AVANAM_DAY_IN_SECONDS );
define( 'AVANAM_YEAR_IN_SECONDS', 365 * AVANAM_DAY_IN_SECONDS );

define( 'AVANAM_AUTOSAVE_INTERVAL', 60 );
...
class Wp_Helper {
    ...
    const PRODUCT_NAME = 'Avanam Builder';
    const PRODUCT_ID   = 'Sample_Product_Testing';

    const SELF_LICENSE_API_URL = 'https://avanam.org/api/presta/v2/';
    const SELF_UPDATE_API_URL = 'https://avanam.org/api/presta/v2/';
    ...
    const REQUEST_LOCK_TTL = AVANAM_MINUTE_IN_SECONDS;
    const REQUEST_LOCK_OPTION_NAME = '_mod_avatheme_partner_api_requests_lock';

    const LICENSE_KEY_OPTION_NAME = 'mod_avatheme_partner_license_key';
    const LICENSE_EMAIL_OPTION_NAME = 'mod_avatheme_partner_license_email';
    const LICENSE_DATA_OPTION_NAME = '_mod_avatheme_partner_license_data';
    ...
    public static function wp_print_jquery() {
        echo '<script src="' . AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/jquery-1.12.4.min.js" type="text/javascript"></script>';
    }

    public static function wp_enqueue_scripts() {
        self::wp_enqueue_script('ui', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/jquery-ui.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('core', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/core.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('widget', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/widget.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('mouse', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/mouse.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('sortable', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/sortable.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('position', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/position.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('menu', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/menu.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('draggable', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/draggable.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('resizable', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/resizable.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('slider', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/slider.min.js', ['jquery'], '1.11.4', true);
        self::wp_enqueue_script('touch', AVANAM_BUILDER_ASSETS_URL . 'lib/jquery/ui/touch-punch.min.js', ['jquery'], '1.11.4', true);

        self::wp_enqueue_script('wp-underscore', AVANAM_BUILDER_ASSETS_URL . 'lib/wp-include/underscore.min.js', [], '', true);
        self::wp_enqueue_script('wp-backbone', AVANAM_BUILDER_ASSETS_URL . 'lib/wp-include/backbone.min.js', [], '', true);
    ...
    public static function wp_request( $url, $args = array() ) {
        ...
        // Force TLS 1.2+ for SSL handshake
    ...
    private static function api_remote_post( $body_args = [] ) {
        ...
        // Only allow API requests in the admin panel (Modules or License page)
        if ( !defined('_PS_ADMIN_DIR_') || (Context::getContext()->controller->controller_name !== 'AdminModules' && Context::getContext()->controller->controller_name !== 'AdminAvanamBuilderLicense') ) {
            return false;
        }
        ...
        $body_args = self::wp_parse_args(
            $body_args,
            [
                'api_version' => AVANAM_BUILDER_VERSION,
                'item_name' => self::PRODUCT_NAME,
                'url' => self::getShopDomainSsl(true),
            ]
        );
        $partner_license_api_url = self::SELF_LICENSE_API_URL;
        ...
        $ava_domain        = str_ireplace( array( 'http://', 'https://' ), '', self::getShopDomainSsl() );
        $current_version   = AVANAM_BUILDER_VERSION;

        $default_query = array(
            'prestashop'    => \AppKernel::VERSION,
        ...
        $data = array(
            'auth_token'       => bin2hex(random_bytes(18 / 2)), // token (unique to each license activation).
            'email'            => $body_args['email'],
            'license_key'      => $body_args['license'],
            'license_domain'   => $ava_domain,
            'platform'         => $ava_domain,
            'request'          => $body_args['api_action'],
            'instance'         => bin2hex(random_bytes(12 / 2)), // Instance ID (unique to each license activation).
            ...
            'packages'         => array(
                'details' => array(
                    'ver'    => ( isset( $current_version ) ? $current_version : '' ),
                    'key'    => ( isset( $body_args['api_action'] ) ? $body_args['api_action'] : '' ),
                    'active' => true,
                ),
            ),
        );
        ...
        $response = self::wp_remote_post(  rtrim($partner_license_api_url, '/') . $request, $remote_post_args );

        if ( self::is_wp_error( $response ) || self::wp_remote_retrieve_response_code( $response ) != 200 ) {
            return false;
        }
        $response = json_decode( self::wp_remote_retrieve_body( $response ), true );
    ...
    public static function api_activate_license( $license_key, $license_email = null ) {
        $body_args = [
            'api_action' => 'activate',
            'license' => $license_key,
            'email' => $license_email,
    ...
    public static function api_deactivate_license( $license_key = null, $license_email = null, $url = null ) {
        $body_args = [
    ...
    public static function api_set_transient( $cache_key, $value, $expiration = '+72 hours' ) {
        $data = [
            'timeout' => strtotime( $expiration, self::current_time() ),
            'value' => json_encode( $value ),
        ];

        Configuration::updateValue( $cache_key, $data );
    }

    private static function api_get_transient( $cache_key ) {
        $cache = Configuration::get( $cache_key );

        if ( empty( $cache['timeout'] ) || self::current_time() > $cache['timeout'] ) {
        }

        return json_decode( Configuration::get( self::LICENSE_DATA_OPTION_NAME, [] ), true );
    }

    public static function api_set_license_data( $license_data, $expiration = null ) {
        // Set main license data
        Configuration::updateValue( self::LICENSE_DATA_OPTION_NAME, json_encode($license_data) );
        // Set fallback data
        Configuration::updateValue( self::LICENSE_DATA_FALLBACK_OPTION_NAME, json_encode($license_data) );
        // Update last check timestamp
        Configuration::updateValue( self::LICENSE_DATA_OPTION_NAME . '_last_check', self::current_time() );
    }

    public static function api_is_license_data_expired() {
        $last_check = Configuration::get( self::LICENSE_DATA_OPTION_NAME . '_last_check', 0 );
        $weekly_interval = AVANAM_WEEK_IN_SECONDS;
        return ( self::current_time() - $last_check ) > $weekly_interval;
    }

    public static function api_get_license_data_from_config() {
        $license_data = Configuration::get( self::LICENSE_DATA_OPTION_NAME );
    ...
    public static function api_get_fallback_license_data() {
        $fallback_data = Configuration::get( self::LICENSE_DATA_FALLBACK_OPTION_NAME );
        if ( empty( $fallback_data ) ) {
            return false;
        }
        return json_decode( $fallback_data, true );
    }

    public static function api_is_request_running( $name ) {
        $requests_lock = Configuration::get( self::REQUEST_LOCK_OPTION_NAME, [] );
        ...
        Configuration::updateValue( self::REQUEST_LOCK_OPTION_NAME, $requests_lock );

        return false;
    }

    public static function api_get_license_data( $force_request = false, $checking = false ) {
        // Force license as valid (DEV / test)
        return [
            'license' => self::STATUS_VALID,
            'success' => true,
            'email' => 'dev@local',
    ...
    public static function api_deactivate( $license_key = null, $license_email = null, $url = null ) {
        $license_data = self::api_deactivate_license( $license_key, $license_email = null, $url );

        Configuration::deleteByName( self::LICENSE_KEY_OPTION_NAME );
        Configuration::deleteByName( self::LICENSE_EMAIL_OPTION_NAME );
        Configuration::deleteByName( self::LICENSE_DATA_OPTION_NAME );
        Configuration::deleteByName( self::LICENSE_DATA_FALLBACK_OPTION_NAME );
    ...
    public static function api_set_license_key( $license_key ) {
        return Configuration::updateValue( self::LICENSE_KEY_OPTION_NAME, $license_key );
    }

    public static function api_get_license_email() {
        return 'dev@local';
    }

    public static function api_set_license_email( $license_email ) {
        return Configuration::updateValue( self::LICENSE_EMAIL_OPTION_NAME, $license_email );
    }
    ...
    public static function api_get_notification() {
        ...
        if ( empty( $license_key ) ) {
            $html = '<div class="alert alert-danger">' . self::__( 'Activate your AvanamBuilder license to unlock feature updates and premium support.', 'elementor' ) . ' <a href="' . self::get_exit_to_dashboard( 'AdminAvanamBuilderLicense' ) . '">' . self::__( '[Click here].', 'elementor' ) . '</a>' . '</div>';
        }else{
            $license_data = self::api_get_license_data();
            ...
            if ( isset($license_data['license']) && self::STATUS_EXPIRED === $license_data['license'] ) {
                $html = '<div class="alert alert-danger">' . self::__( 'Your License Has Expired. Renew your license today to keep getting feature updates, premium support and unlimited access to the template library.', 'elementor' ) . ' <a href="' . self::get_exit_to_dashboard( 'AdminAvanamBuilderLicense' ) . '">' . self::__( '[Click here].', 'elementor' ) . '</a>' . '</div>';
            }

            if ( isset($license_data['license']) && self::STATUS_SITE_INACTIVE === $license_data['license'] ) {
                $html = '<div class="alert alert-danger">' . self::__( 'Your license key doesn\'t match your current domain. This is most likely due to a change in the domain URL of your site (including HTTPS/SSL migration). Please deactivate the license and then reactivate it again.', 'elementor' ) . ' <a href="' . self::get_exit_to_dashboard( 'AdminAvanamBuilderLicense' ) . '">' . self::__( '[Click here].', 'elementor' ) . '</a>' . '</div>';
            }

            if ( isset($license_data['license']) && self::STATUS_INVALID === $license_data['license'] ) {
                $html = '<div class="alert alert-danger">' . self::__( 'Your license key doesn\'t match your current domain. This is most likely due to a change in the domain URL of your site (including HTTPS/SSL migration). Please deactivate the license and then reactivate it again.', 'elementor' ) . ' <a href="' . self::get_exit_to_dashboard( 'AdminAvanamBuilderLicense' ) . '">' . self::__( '[Click here].', 'elementor' ) . '</a>' . '</div>';
            }

            if ( isset($license_data['license']) && self::STATUS_DISABLED === $license_data['license'] ) {
                $html = '<div class="alert alert-danger">' . self::__( 'Your license key has been cancelled (most likely due to a refund request). Please consider acquiring a new license.', 'elementor' ) . ' <a href="' . self::get_exit_to_dashboard( 'AdminAvanamBuilderLicense' ) . '">' . self::__( '[Click here].', 'elementor' ) . '</a>' . '</div>';
            }
        }


        // Auto-check for updates on page load
        $updateAvailable = self::checkForUpdate();

        $context = Context::getContext();

        // If the user clicks "Update Now"
        if (Tools::isSubmit('update_module_now')) {
            if ($updateAvailable && self::downloadAndInstallUpdate()) {
                // Trigger the upgrade process right after updating files
                self::checkAndUpgrade();

                // Show success message
                //self::$output .= $this->displayConfirmation(self::__('Module successfully updated and upgraded!'));
                $context->controller->confirmations[] = self::__('Module successfully updated and upgraded!');
                Tools::redirectAdmin($this->context->link->getAdminLink('AdminModules', true).'&configure='.$this->name);
            } else {
                // Show error message if something goes wrong
                //self::$output .= $this->displayError(self::__('Failed to update the module. Please try again.'));
                $context->controller->errors[] = self::__('Failed to update the module. Please try again.');
            }
        }

        if ($updateAvailable) {

            //$this->context->controller->warnings[] = $this->trans('A new version of the AvanamBuilder is available.', [], 'Modules.Avanambuilder.Admin');
            $context->controller->warnings[] = self::__('A new version of the AvanamBuilder is available.');
        }

        return $html . self::renderUpdateButton($updateAvailable);
    }
    ...
    public static function get_permalink_template( $elementor_library ) {
        return Context::getContext()->link->getModuleLink( 'avanambuilder', 'preview', [ 'elementor_library' => $elementor_library ], true );
    }
    ...
    public static function get_permalink( $params = [] ) {
        ...
        switch ( $post_type ) {
            case 'hook':
                if ( $key_related == 'displayProductSidebar' || $key_related == 'displayProductSummary' || $key_related == 'displayFooterProduct' || $key_related == 'displayLeftColumnProduct' || $key_related == 'displayRightColumnProduct' || $key_related == 'displayProductAccessories' || $key_related == 'displayProductSameCategory' ) {
    ...
    public static function get_ajax_editor( $params = [] ) {
        ...
        return $context->link->getModuleLink( 'avanambuilder', 'ajax_editor', $params, null, $id_lang, $id_shop );
    }
    ...
    public static function is_admin() {
        return !empty( Context::getContext()->controller->name ) && Context::getContext()->controller->name == 'AvanamBuilderEditor';
    }
    ...
    {
        return Tools::hash('AvanamBuilderEditor'.(isset(Context::getContext()->employee->id) ? (int)Context::getContext()->employee->id :
                (int)Tools::getValue('id_employee')));
    }

    public static function getRelatedByKey() {
        $sql = 'SELECT * 
		        FROM `'._DB_PREFIX_.'avanam_builder_related` bc
				LEFT JOIN `'._DB_PREFIX_.'avanam_builder_related_shop` bcs
    ...
    public static function getTemplatesLocal() {
        $sql = 'SELECT * 
		        FROM `'._DB_PREFIX_.'avanam_builder_template`';
    ...
    public static function get_post_meta( $id, $key = '', $single = false ) {
        $db = Db::getInstance();
        $table = _DB_PREFIX_ . 'avanam_builder_meta';
    ...
    public static function update_post_meta( $id, $key, $value = '' ) {
        ...
        $id_meta = $db->getValue("SELECT id_avanam_builder_meta FROM $table WHERE id = {$data['id']} AND name = '{$data['name']}'");

        if ( $id_meta ) {
            $data['id_avanam_builder_meta'] = $id_meta;
            $type = Db::REPLACE;
        } else {
            $type = Db::INSERT;
        }
    ...
    public static function delete_post_meta( $id, $key ) {
        ...
        return $db->delete('avanam_builder_meta', "id = $id AND name = '$key'");
    }
    ...
    public static function human_time_diff( $from, $to = 0 ) {
        ...
        if ( $diff < AVANAM_MINUTE_IN_SECONDS ) {
            $secs = $diff;
            if ( $secs <= 1 ) {
                $secs = 1;
            }
        ...
        } elseif ( $diff < AVANAM_HOUR_IN_SECONDS && $diff >= AVANAM_MINUTE_IN_SECONDS ) {
            $mins = round( $diff / AVANAM_MINUTE_IN_SECONDS );
            if ( $mins <= 1 ) {
                $mins = 1;
            }
            /* translators: Time difference between two dates, in minutes (min=minute). %s: Number of minutes. */
            $since = sprintf( self::_n( '%s min', '%s mins', $mins ), $mins );
        } elseif ( $diff < AVANAM_DAY_IN_SECONDS && $diff >= AVANAM_HOUR_IN_SECONDS ) {
            $hours = round( $diff / AVANAM_HOUR_IN_SECONDS );
            if ( $hours <= 1 ) {
                $hours = 1;
            }
            /* translators: Time difference between two dates, in hours. %s: Number of hours. */
            $since = sprintf( self::_n( '%s hour', '%s hours', $hours ), $hours );
        } elseif ( $diff < AVANAM_WEEK_IN_SECONDS && $diff >= AVANAM_DAY_IN_SECONDS ) {
            $days = round( $diff / AVANAM_DAY_IN_SECONDS );
            if ( $days <= 1 ) {
                $days = 1;
            }
            /* translators: Time difference between two dates, in days. %s: Number of days. */
            $since = sprintf( self::_n( '%s day', '%s days', $days ), $days );
        } elseif ( $diff < AVANAM_MONTH_IN_SECONDS && $diff >= AVANAM_WEEK_IN_SECONDS ) {
            $weeks = round( $diff / AVANAM_WEEK_IN_SECONDS );
            if ( $weeks <= 1 ) {
                $weeks = 1;
            }
            /* translators: Time difference between two dates, in weeks. %s: Number of weeks. */
            $since = sprintf( self::_n( '%s week', '%s weeks', $weeks ), $weeks );
        } elseif ( $diff < AVANAM_YEAR_IN_SECONDS && $diff >= AVANAM_MONTH_IN_SECONDS ) {
            $months = round( $diff / AVANAM_MONTH_IN_SECONDS );
            if ( $months <= 1 ) {
                $months = 1;
            }
            /* translators: Time difference between two dates, in months. %s: Number of months. */
            $since = sprintf( self::_n( '%s month', '%s months', $months ), $months );
        } elseif ( $diff >= AVANAM_YEAR_IN_SECONDS ) {
            $years = round( $diff / AVANAM_YEAR_IN_SECONDS );
            if ( $years <= 1 ) {
                $years = 1;
            }
    ...
    public static function set_global_var() {
        ...
        switch ( $post_type ) {
            ...
            case 'blog':
                ...
                if( !$related ){
                    ...
                    $related = new \AvanamBuilderRelated();
        ...
        $post = new \AvanamBuilderPost( $id_post, $id_lang );
    ...
    public static function checkForUpdate()
    {
        ...
        $currentVersion = AVANAM_BUILDER_VERSION;
        $lastCheck = Configuration::get('AVABUILDER_LAST_UPDATE_CHECK');
        $latestVersion = Configuration::get('AVABUILDER_LATEST_VERSION');
        $currentTime = time();
        $checkInterval = 3.5 * 24 * 60 * 60; // 3.5 days (twice a week)

        /*echo $lastCheck;
        echo $currentVersion;
        echo $latestVersion;
        exit;*/
        // If the latest version exists and is newer than the current version, return true immediately
        if ($latestVersion && version_compare($currentVersion, $latestVersion, '<') && !Tools::getValue('check_update_now')) {
            return true;
        }


        // Manual update trigger via URL
        if (Tools::getValue('check_update_now') || !$lastCheck || ($currentTime - $lastCheck) >= $checkInterval) {

            $partner_update_api_url = self::SELF_UPDATE_API_URL;
            if (defined('PARTNER_UPDATE_API_URL')) {
                $partner_update_api_url = PARTNER_UPDATE_API_URL;
            }

            $response = @file_get_contents($partner_update_api_url);

            $context = Context::getContext();

            if (!$response) {

                if (strpos($partner_update_api_url, 'avanam.org') !== false) {
                    return false;  // Return immediately if "avanam.org" is in the URL
                }
                //self::$output .= $this->displayError(self::__('Failed to connect to the update server.'));
                $context->controller->errors[] = self::__('Failed to connect to the update server.');
                return false;
            }
            ...
            if (!$updateInfo || !isset($updateInfo['version'])) {
                //self::$output .= $this->displayError(self::__('Invalid update response.'));
                $context->controller->errors[] = self::__('Invalid update response.');
                return false;
            }

            $latestVersion = trim($updateInfo['version']);

            // Cache the latest version and timestamp
            Configuration::updateValue('AVABUILDER_LAST_UPDATE_CHECK', $currentTime);
            Configuration::updateValue('AVABUILDER_LATEST_VERSION', $latestVersion);

            return version_compare($currentVersion, $latestVersion, '<');
        }

        return false; // Skip checking if the time interval hasn’t passed
    }
    ...
    public static function downloadAndInstallUpdate()
    {
        ...
        $response = @file_get_contents( $partner_update_api_url . '?key='.bin2hex(random_bytes(18 / 2)) );
        if ($response === false) {
            return false;
        }

        $updateInfo = json_decode($response, true);
        $zipUrl = $updateInfo['download_url'];
        $zipFile = _PS_MODULE_DIR_ . 'avanambuilder.zip';
    ...
    public static function renderUpdateButton($updateAvailable)
    {
        $latestVersion = Configuration::get('AVABUILDER_LATEST_VERSION', AVANAM_BUILDER_VERSION); // Cached version info
        $has_license = false;

        // License check
        $license_data = self::api_get_license_data();
        if (isset($license_data['license']) && self::STATUS_VALID === $license_data['license']) {
            $has_license = true;
        }

        // Define update and manual check links
        $updateLink = $_SERVER['REQUEST_URI'] .  ( !isset($_GET['update_module_now']) ? '&update_module_now=1' : '');//'&update_module_now=1';
        $manualCheckLink = $_SERVER['REQUEST_URI'] .  ( !isset($_GET['check_update_now']) ? '&check_update_now=1' : '');//'&check_update_now=1';

        // Default button when license is inactive
        $updateOpt = !$has_license ? '<a href="' . Context::getContext()->link->getAdminLink('AdminAvanamBuilderLicense') . '" class="btn btn-primary" style="margin-right: 15px;">
				' . self::__('Activate license to receive updates') . '
			</a>' : '';

        // Show "Update Now" if an update is available
        if ($updateAvailable && $has_license) {
            $updateOpt = '<a href="' . $updateLink . '" class="btn btn-primary" style="margin-right: 15px;">
				' . self::__('Update Now') . '
			</a>';
        }
        ...
        $statusMessage = $updateAvailable
            ? '<h3>' . self::__('AvanamBuilder Update Available!') . '</h3>'
            : '<h3>' . self::__('AvanamBuilder Status') . '</h3>';

        $successMessage = $has_license && $latestVersion ==  AVANAM_BUILDER_VERSION
            ? '<p class="alert alert-success" style="margin-top: 20px;">' . self::__('AvanamBuilder is up to date.') . '</p>'
            : '';

        $checkUpdateButton = $has_license
            ? '<a href="' . $manualCheckLink . '" class="btn btn-primary">' . self::__('Check for Updates') . '</a>'
            : '';

        return '<div class="panel">
					' . $statusMessage . '
					<p><strong>' . self::__('Current version:') . '</strong> ' . AVANAM_BUILDER_VERSION . '</p>
					<p><strong>' . self::__('Latest version:') . '</strong> ' . $latestVersion . '</p>
					' . $updateOpt . $checkUpdateButton . $successMessage . '
				</div>';
    }

    public static function checkAndUpgrade()
    {
        // using AVANAM_BUILDER_VERSION because this function called only after downloaded files, so latest version files.
        $latestVersion = AVANAM_BUILDER_VERSION;
        $latestVersionSani = str_replace('.', '_', AVANAM_BUILDER_VERSION);
        $functionName = "upgrade_module_$latestVersionSani";

        $upgradeFile = AVANAM_BUILDER_PATH . 'upgrade/upgrade-'.$latestVersion.'.php';

        if (file_exists($upgradeFile)) {
            require_once($upgradeFile);

            if (function_exists($functionName)) {
                $functionName($this); //upgrade_module_1_1_0($this);
            }
            //$this->_output .= $this->displayConfirmation($this->l('Module upgraded successfully!'));
            $context = Context::getContext();
            $context->controller->confirmations[] = self::__('Module upgraded successfully!');
        }
    }
