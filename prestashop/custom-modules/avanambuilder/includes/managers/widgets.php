...
use AvanamBuilder\Core\Utils\Exceptions;
...
class Widgets_Manager {
	...
	*/
	private function init_widgets() {
		...
		include( AVANAM_BUILDER_PATH . 'includes/widgets/traits/nav.php' );
		...
		$this->_ava_widget[] = 'ava-products';
		
		$this->_ava_widget[] = 'ava-products-tabs';

		$this->_ava_widget[] = 'ava-sign-in';

		$this->_ava_widget[] = 'ava-site-logo';
		
		$this->_ava_widget[] = 'ava-site-title';

		$this->_ava_widget[] = 'ava-shopping-cart';

		if( \Module::isEnabled('avanamsearchbar') ) {
			$this->_ava_widget[] = 'ava-search';
		}

		if( \Module::isEnabled('blockwishlist') ) {
			$this->_ava_widget[] = 'ava-wishlist';
		}

		if( \Module::isEnabled('avanamcompare') ) {
			$this->_ava_widget[] = 'ava-compare';
		}
		
		if( \Module::isEnabled('avanamegamenu') ) {
			$this->_ava_widget[] = 'ava-menu';
		}

		if( \Module::isEnabled('avanamverticalmenu') && !\Module::isEnabled('avanamegamenu')) {
			$this->_ava_widget[] = 'ava-menu';
		}
		
		$this->_widget_types = [];
		
		$build_widgets_filename = array_merge( $build_widgets_filename, $this->_ava_widget );

		foreach ( $build_widgets_filename as $widget_filename ) {
			include( AVANAM_BUILDER_PATH . 'includes/widgets/' . $widget_filename . '.php' );
	...
	*/
	private function require_files() {
		require AVANAM_BUILDER_PATH . 'includes/base/widget-base.php';
	}
	...
	*/
	public function get_widget_types_config() {
		...
		foreach ( $this->get_widget_types() as $widget_key => $widget ) {
			...
			if( in_array( $widget_key, $this->_ava_widget ) && !Wp_Helper::api_is_license_active() ){
