...
class Widget_Countdown extends Widget_Base {
	...
	public function get_keywords() {
		return [ 'countdown', 'number', 'timer', 'time', 'date', 'evergreen', 'ava' ];
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'description' => sprintf( Wp_Helper::__( 'Date set according to your timezone: %s.', 'elementor' ), Utils::get_timezone_string() ),
				'condition' => [
	...
	private function init_default_countdown_labels($instance = null) {
	...
	public function get_default_countdown_labels($instance = null) {
		if ( ! $this->_default_countdown_labels ) {
			$this->init_default_countdown_labels($instance);
		}
	...
	private function render_countdown_item( $instance, $label, $part_class ) {
		...
		if ( $instance['show_labels'] ) {
			$default_labels = $this->get_default_countdown_labels($instance);
	...
	private function get_evergreen_interval( $instance ) {
		$hours = empty( $instance['evergreen_counter_hours'] ) ? 0 : ( $instance['evergreen_counter_hours'] * AVANAM_HOUR_IN_SECONDS );
		$minutes = empty( $instance['evergreen_counter_minutes'] ) ? 0 : ( $instance['evergreen_counter_minutes'] * AVANAM_MINUTE_IN_SECONDS );
