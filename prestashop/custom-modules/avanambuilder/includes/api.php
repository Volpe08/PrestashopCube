...
class Api {
	 ...
	 */
	public static $api_info_url = 'https://avanam.org/api/presta/v2/';
	 ...
	 */
	private static $api_feedback_url = 'https://avanam.org/api/presta/v2/feedback';
	 ...
	 */
	private static $api_get_template_content_url = 'https://avanam.org/api/presta/templates?id=%id';
	...
	private static function get_info_data( $force_update = false ) {
		$cache_key = 'elementor_remote_info_api_data_' . AVANAM_BUILDER_VERSION;
		...
		if ( $force_update || false === $info_data ) {
			...
			if ( Wp_Helper::is_wp_error( $response ) || 200 !== (int) Wp_Helper::wp_remote_retrieve_response_code( $response ) ) {
				Wp_Helper::set_transient( $cache_key, [], 2 * AVANAM_HOUR_IN_SECONDS );
			...
			Wp_Helper::set_transient( $cache_key, $info_data, 12 * AVANAM_HOUR_IN_SECONDS );
		}
