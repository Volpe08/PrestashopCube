...
class Autoloader {
	...
	private static function load_class( $relative_class_name ) {
		...
		if ( isset( $classes_map[ $relative_class_name ] ) ) {
			$filename = AVANAM_BUILDER_PATH . '/' . $classes_map[ $relative_class_name ];
		} else {
			$filename = strtolower(
				preg_replace(
					[ '/([a-z])([A-Z])/', '/_/', '/\\\/' ],
			...
			$filename = AVANAM_BUILDER_PATH . $filename . '.php';
		}
	...
	private static function autoload( $class ) {
		...
		if ( $has_class_alias ) {
			...
			preg_match( '/^[0-9]+\.[0-9]+/', AVANAM_BUILDER_VERSION, $current_version );
