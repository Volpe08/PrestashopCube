...
namespace AvanamBuilder\TemplateLibrary;

use AvanamBuilder\Api;
use AvanamBuilder\Core\Common\Modules\Ajax\Module as Ajax;
use AvanamBuilder\Core\Settings\Manager as SettingsManager;
use AvanamBuilder\TemplateLibrary\Classes\Import_Images;
...
class Manager {
	...
	public function import_template( array $data ) {
		...
		// Check active child theme		
		$activeTheme = \Context::getContext()->shop->theme->getName();
		$activeThemeName = \Context::getContext()->shop->theme->get('display_name');
		$parentTheme = \Context::getContext()->shop->theme->get('parent');

		if ($parentTheme == 'avanamclassic') {
			...
			if (!empty($customVariables)) {
				...
				$old_url = $customVariables['demo_url'] ?? '';
				$old_esc_url = str_replace('/', '\\/', $old_url);

				// Get new store URL and its path
				$new_url = \Context::getContext()->link->getBaseLink();
				$new_esc_url = str_replace('/', '\\/', $new_url);

				$file_content = str_replace($old_url, $new_url, $file_content);
				$file_content = str_replace($old_esc_url, $new_esc_url, $file_content);
