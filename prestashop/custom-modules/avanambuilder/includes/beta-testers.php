...
class Beta_Testers {
	...
	private function get_beta_version() {
		...
		if ( false === $beta_version ) {
			...
			set_site_transient( $this->transient_key, $beta_version, 6 * AVANAM_HOUR_IN_SECONDS );
		}
	...
	public function check_version( $transient ) {
		...
		if ( 'false' !== $beta_version && version_compare( $beta_version, AVANAM_BUILDER_VERSION, '>' ) ) {
			...
			$transient->response[ AVANAM_BUILDER_PLUGIN_BASE ] = $response;
		}
