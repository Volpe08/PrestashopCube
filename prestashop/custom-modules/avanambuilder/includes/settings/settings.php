...
use AvanamBuilder\Core\Settings\General\Manager as General_Settings_Manager;
use AvanamBuilder\Core\Settings\Manager;
				...
				<div class="e-getting-started__box postbox">
					...
					<div class="e-getting-started__content">
						...
						<div class="e-getting-started__actions e-getting-started__content--narrow">
							...
							<a href="https://avanam.org/presta/getting-started/" target="_blank" class="button button-secondary button-hero"><?php echo Wp_Helper::__( 'Get the Full Guide', 'elementor' ); ?></a>
						</div>
					</div>
				</div>
			</div>
						...
						'fields' => [
							'allow_tracking' => [
								...
								'field_args' => [
									...
									'value' => 'yes',
									'default' => '',
									'sub_desc' => Wp_Helper::__( 'Opt-in to our anonymous plugin data collection and to updates. We guarantee no sensitive data is collected.', 'elementor' ) . sprintf( ' <a href="%1$s" target="_blank">%2$s</a>', 'https://avanam.org/presta/usage-data-tracking/', Wp_Helper::__( 'Learn more.', 'elementor' ) ),
								],
	...
	public function __construct() {
		...
		foreach ( Responsive::get_editable_breakpoints() as $breakpoint_key => $breakpoint ) {
			foreach ( [ 'add', 'update' ] as $action ) {
				Wp_Helper::add_action( "{$action}_option_elementor_viewport_{$breakpoint_key}", [ 'AvanamBuilder\Core\Responsive\Responsive', 'compile_stylesheet_templates' ] );
			}
		}
	}
}
