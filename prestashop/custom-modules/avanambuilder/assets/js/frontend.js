  ...
  onInit: function onInit() {
    ...
    var slickOptions = {
      ...
      responsive: [{
        ...
        settings: {
          slidesToShow: +elementSettings.slides_to_show_laptop || defaultLGDevicesSlidesCount,
          slidesToScroll: +elementSettings.slides_to_scroll_laptop || defaultLGDevicesSlidesCount
        }
      }, {
...
jQuery(function($){
    ...
    var $cartelement = $('.elementor-widget-ava-shopping-cart');
    var $body = $('body');
    var $overlay = $('.cart_overlay');
    var elements = {};
    elements.$container = $cartelement.find( '.elementor-cart__container');
    elements.$toggle = $cartelement.find('.elementor-cart__toggle .elementor-button');
    elements.$closeButton = $cartelement.find('.elementor-cart__close-button');

    // Initialize toggle and container from prestashop's JS cart object on page load (single direct read).
    // If `prestashop.cart` is available at DOM ready, update the toggle and cart container immediately.
    (function initCartFromPrestashopOnce(){
      if (!elements.$toggle || !elements.$toggle.length) return;
      try{
        var cart = (window.prestashop && window.prestashop.cart) || window.cart;
        if (!cart) return;

        // Update counter
        if (typeof cart.products_count !== 'undefined'){
          elements.$toggle.find('.elementor-cart-icon').attr('data-counter', cart.products_count).data('counter', cart.products_count);
        }

        // Update toggle text: show subtotal when available, otherwise a zero amount fallback
        try {
          var subtotalText = (cart.subtotals && cart.subtotals.products && cart.subtotals.products.value) ? cart.subtotals.products.value : (cart.products_count ? '' : '$0.00');
          elements.$toggle.find('.elementor-button-text').html(subtotalText);
        } catch (e) { /* noop */ }

        // If Prestashop provided full products data on window.cart/prestashop.cart, render the live cart details into the container
        if (elements.$container && elements.$container.length && cart.products && cart.products.length) {
          var gift = elements.$container.find('.elementor-cart__products').data('gift');
          var $products = $();
          var remove_item_icon = $cartelement.data('settings').remove_item_icon || {};
          ...
          var $prod = $(
              ...
              '</div>'
                ...
                cover = product.cover || (window.prestashop && prestashop.urls && prestashop.urls.no_picture_image) || product.cover;
            ...
            $('<img>').appendTo($prod.find('.elementor-cart__product-image')).attr({
                src: (cover && cover.bySize && cover.bySize.cart_default && cover.bySize.cart_default.url) || (cover && cover.small && cover.small.url) || (cover && cover.url) || '',
                alt: cover && cover.legend || ''
            });
            ...
            for (var label in product['attributes']) {
                if (!product['attributes'].hasOwnProperty(label)) continue;
          ...
          // Update cart DOM
          elements.$container.find('.elementor-cart__products')
            .empty()
            .append($products)
          ;
          elements.$container.find('.elementor-cart__empty-message')
            .toggleClass('elementor-hidden', !!cart.products_count)
          ;
          elements.$container.find('.elementor-cart__inner')
            .toggleClass('elementor-hidden', !cart.products_count)
          ;
          elements.$container.find('.elementor-cart__summary').html(
          ...
          elements.$container.find('.elementor-alert-warning')
            .toggleClass('elementor-hidden', !cart.minimalPurchaseRequired)
            .html('<span class="elementor-alert-description">' + cart.minimalPurchaseRequired + '</span>');
          elements.$container.find('.elementor-button--checkout')
            .toggleClass('av-disabled', cart.minimalPurchaseRequired || !cart.products_count)
          ;
        } else if (elements.$container && elements.$container.length) {
          // No products - ensure empty state visibility
          var hasProducts = (cart.products && cart.products.length) || false;
          elements.$container.find('.elementor-cart__empty-message').toggleClass('elementor-hidden', hasProducts);
          elements.$container.find('.elementor-cart__inner').toggleClass('elementor-hidden', !hasProducts);
        }
      } catch (e) {
    ...
    elements.$toggle.on('click', function (event) {
        if (!elements.$toggle.hasClass('elementor-cart-hidden')) {
            event.preventDefault();
            $container.toggleClass('elementor-cart--shown');
            $body.toggleClass('body-scrollbar-fixer');
    ...
    $container.on('click', function (event) {
        if ($container.hasClass('elementor-cart--shown') && $container[0] === event.target) {
            ...
            $body.removeClass('body-scrollbar-fixer');
    ...
    $container.on('click', '.elementor-cart__product-remove a', function (event) {
        ...
        $(this).closest('.elementor-cart__product').addClass('av-disabled');
    ...
    prestashop.on('updateCart', function(data) {
        ...
        if ($cartelement.data('settings').action_show_modal && 'add-to-cart' === data.reason.linkAction && !data.resp.hasError) {

            $.post(
                $cartelement.data('settings').modal_url,
                {
                    ajax: true,
        ...
        // Update toggle
        elements.$toggle.find('.elementor-button-text')
            .html(cart.subtotals.products.value)
        ;
        elements.$toggle.find('.elementor-cart-icon')
        ...
        $container.find('.elementor-cart__empty-message')
            .toggleClass('elementor-hidden', !!cart.products_count)
        ;
        $container.find('.elementor-cart__inner')
          .toggleClass('elementor-hidden', !cart.products_count)
        ;
        ...
        if ($cartelement.data('settings').action_open_cart && 'add-to-cart' === data.reason.linkAction && !data.resp.hasError) {
            elements.$container.hasClass('elementor-cart--shown') || elements.$toggle.triggerHandler('click');
        }
    });
});
...
jQuery(function($){
    ...
    var wishlistMut = new MutationObserver(function(mutations, mut){

        if($( ".wishlist-toast" ).hasClass( "success" )  && $( ".wishlist-toast" ).hasClass( "isActive" ) ){

            if ($( ".btn-canvas-wishlist" ).data('ajaxurl')) {

                $.post(
                    $( ".btn-canvas-wishlist" ).data('ajaxurl'),
                    {
                        ajax: true,
                        action: 'updateWishlist',
                    },
                    function (resp) {
                        $('.elementor-wish-icon.wishlist').attr('data-counter', resp = resp.replace(/"/g, ''));
                        $('#blockcart-modal').remove();
                        $('.ava-modal-wishlist .modal-title').html($('.wishlist-toast-text').html());
				                $('.ava-modal-wishlist').modal('show');
                        //$(document.body).append(resp.modal).children('#blockcart-modal').modal();
                    },
                    'json'
                );
    ...
    var wishlistToast = document.querySelector(".wishlist-toast");
    if (wishlistToast) {
        wishlistMut.observe(wishlistToast, {
            'attributes': true,
